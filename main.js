let cvsString =
  "ID,Name,Occupation,Age\n42,Bruce,Knight,41\n57,Bob,Fry Cook,19\n63,Blaine,Quiz Master,58\n98,Bill,Doctor's Assistant,26";
//   "Index,Mass (kg),Spring 1 (m),Spring 2 (m)\n1,0.00,0.050,0.050\n2,0.49,0.066,0.066\n3,0.98,0.087,0.080\n4,1.47,0.116,0.108\n5,1.96,0.142,0.138\n6,2.45,0.166,0.158\n7,2.94,0.193,0.174\n8,3.43,0.204,0.192\n9,3.92,0.226,0.205\n10,4.41,0.238,0.23";

let initialArray = cvsString.split("\n");
let numColumns = initialArray[0].split(",").length;
let twoDimenArray = [];
let objArray = [];

for (let i = 0; i < initialArray.length; i++) {
  stringArray = initialArray[i].split(",");
  twoDimenArray[i] = stringArray;
}

for (let i = 1; i < twoDimenArray.length; i++) {
  let dataObj = {};
  for (let j = 0; j < twoDimenArray[i].length; j++) {
    dataObj[twoDimenArray[0][j].toLowerCase()] = twoDimenArray[i][j];
  }
  objArray[i - 1] = dataObj;
}

console.log(objArray);

// Part 4
objArray.pop();
console.log(objArray);

objArray[1] = {
  id: "48",
  name: "Barry",
  occupation: "Runner",
  age: "25",
};
console.log(objArray);

objArray.push({
  id: "7",
  name: "Bilbo",
  occupation: "None",
  age: "111",
});

console.log(objArray);

let avgAge = 0;

for (let i = 0; i < objArray.length; i++) {
  let numAges = 1 / objArray.length;
  let age = objArray[i].age;
  avgAge += numAges * age;
}

console.log(avgAge);

// Part 5
let returnToCSV = "";
returnToCSV += Object.keys(objArray[0]).toString() + "\n";
for (let i = 0; i < objArray.length; i++) {
  // let objString = `${objArray[id],objArray[name],objArray[occupation],objArray[age],}`
  if (i != objArray.length - 1) {
    returnToCSV += Object.values(objArray[i]) + "\n";
  } else {
    returnToCSV += Object.values(objArray[i]);
  }
}
console.log(returnToCSV);